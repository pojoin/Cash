package com.cash.common.util
{
	...
	public class EntityManager extends EventDispatcher
	{
		...
		*/
		...
		public function search(c:Class,condition:String):ArrayCollection{
			// If not yet done, load the metadata for this class
			loadMetadataForCondition(c,condition);
			var stmt:SQLStatement = map[c].search;
			trace(stmt.text);
			stmt.execute();
			// Return typed objects
			var result:Array = stmt.getResult().data;
...
//			}
		...
		//根据条件
		private function loadMetadataForCondition(c:Class,condition:String):void
		{
			map[c] = new Object();
			var xml:XML = describeType(new c());
			var table:String = xml.metadata.(@name=="Table").arg.(@key=="name").@value;
			...
			stmt= new SQLStatement();
			stmt.sqlConnection = sqlConnection;
			stmt.text = "SELECT * FROM " + table + " WHERE " + condition;
			map[c].search = stmt;
		...
		//条件查询多少条 
		public function getQueryCountNum(c:Class,condition:String):int{
			loadMetadataForCondition(c,condition);
			var xml:XML = describeType(new c());
			var table:String = xml.metadata.(@name=="Table").arg.(@key=="name").@value;
